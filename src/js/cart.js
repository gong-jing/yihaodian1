!function($){
	...
	function goodslist(id,count){
		$.ajax({
			url:'http://localhost/1907%20gj/yihaodian/php/likedata.php',//获取所有的接口数据
			dataType:'json'
		}).done(function(data){
	...
	if(myobj.getcookie('cookiesid') && myobj.getcookie('cookienum')){
		var s=myobj.getcookie('cookiesid').split(',');//数组sid
		var n=myobj.getcookie('cookienum').split(',');//数组num
	...
	function cookietoarray(){
		if(myobj.getcookie('cookiesid') && myobj.getcookie('cookienum')){
			arrsid=myobj.getcookie('cookiesid').split(',');//cookie商品的sid  
			arrnum=myobj.getcookie('cookienum').split(',');//cookie商品的num
		}
	}
	function setcookie(obj) { //obj:当前操作的对象
	    ...
	    myobj.addcookie('cookienum', arrnum.toString(), 7);
	}
	...
	function delgoodslist(sid, arrsid) {//sid：当前删除的sid，arrsid:cookie的sid的值
	    ...
	    myobj.addcookie('cookiesid', arrsid.toString(), 7);//添加cookie
	    myobj.addcookie('cookienum', arrnum.toString(), 7);//添加cookie
	}
